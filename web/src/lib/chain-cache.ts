 ...
 * written to disk.
 *
 * ~512 blocks is on the order of ten minutes on this chain. The positions this app reads
 * are hours to months old, so in practice nothing a scan cares about is inside the
 * window and the depth costs nothing; it exists so that a scan run seconds after a mint
 * does not bake the tip into a cache that outlives it. Raising it is cheap (one narrow
 * tail query per key per visit); lowering it below a chain's real reorg depth is not.
 */
export const REORG_DEPTH = 512n;

/**
 * Key namespace. The chain id keeps two networks from sharing an entry; the `v1` is the
 * SEMANTIC version -- bump it when the meaning of a stored value changes without its
 * shape changing (a decoding fix, a different event, a widened query), which the
 * IndexedDB version bump in idb.ts would not catch.
 */
const NS = `v1:${ROBINHOOD_CHAIN.chainId}`;
...
export function cachedBlockTimestamp(
  blockNumber: bigint,
  fetcher: (blockNumber: bigint) => Promise<number>,
): Promise<number> {
 ...
 */
export function cachedReceipt<T extends { blockNumber: bigint }>(
  hash: string,
 ...
 */
export function cachedTokenMetaPersistent(
  address: string,
  fetcher: (address: string) => Promise<TokenMeta>,
): Promise<TokenMeta> {
  return cachedTokenMeta(address, (a) =>
    cachedPoint(`tokenmeta:${a.toLowerCase()}`, () => fetcher(a), () => true));
}
 ...
 * tested.
 */
export function cachedLogRange<L extends { blockNumber: bigint | null; logIndex: number | null }>(
  key: string,
  from: bigint,
  to: bigint,
  fetchRange: (from: bigint, to: bigint) => Promise<L[]>,
): Promise<L[]> {
  ...
  return cachedByKey(`range:${key}:${from}:${to}`, async () => {
    const store = await getStore();
 ...
 */
export async function cachedLogsById<L extends { blockNumber: bigint | null; logIndex: number | null }>(
  keyPrefix: string,
  ids: readonly bigint[],
  from: bigint,
  to: bigint,
  fetchIds: (ids: bigint[], from: bigint, to: bigint) => Promise<L[]>,
  idOf: (log: L) => bigint,
 ...
 *
 * All three layers, not just the disk: leaving the in-scan promise caches populated would
 * make the button look broken, because the page would answer from those before the store
 * was ever consulted.
 */
