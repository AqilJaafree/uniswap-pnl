 ...
 * imported — chain-v4.ts is built INSIDE chain.ts's factory (see chain.ts's
 * createChainClient, which constructs `shared` and calls createV4Client(chain, cache,
 * shared)), so a module-level import here would be circular at runtime.
 */
...
export function createV4Client(chain: ChainConfig, cache: ChainCache, shared: SharedPlumbing): V4Client {
  ...
  function slot0TickAt(meta: V4Meta, blockNumber: bigint): Promise<number | null> {
    return cache.cachedPoint(
      ...
      (tick) => tick !== null && cache.isFinal(blockNumber),
    );
  }
  ...
  async function tokenMeta(addr: string): Promise<{ dec: number; sym: string }> {
    ...
    return cache.cachedTokenMetaPersistent(addr, async (a) => {
  ...
  function swapTickAt(meta: V4Meta, head: bigint, blockNumber: bigint): Promise<number | null> {
    return cache.cachedPoint(
      ...
      (t) => t !== null && cache.isFinal(blockNumber),
    );
  }
  ...
  function feeGrowthAt(meta: V4Meta, blockNumber: bigint): Promise<{ fg0: bigint; fg1: bigint } | null> {
    return cache.cachedPoint(
      ...
      (fg) => fg !== null && cache.isFinal(blockNumber),
    );
  }
  ...
  async function fetchV4Lifecycle(tokenId: bigint, meta: V4Meta, head: bigint): Promise<{ raw: V4RawEvent[]; tsByBlock: Map<bigint, number> }> {
    ...
    const mine = await cache.cachedLogRange<ModifyPoint>(`v4:modify:${meta.poolId}:${tokenId}`, meta.mintBlock, head, async (from, to) =>
  ...
  async function computePositionPnLV4(tokenId: bigint, mintBlock: bigint, ctx?: OwnerContext): Promise<PositionPnL> {
    const meta = await fetchMeta(tokenId, mintBlock);
    const num = pickNumeraire(chain, meta.poolKey.currency0, meta.poolKey.currency1, meta.sym0, meta.sym1);
    ...
    return {
      ...
      priceT1perT0, priceBasis, txHashes, soldAt, exitTx: exitTxHash(events), gasEth,
      gasKind: chain.gasIsUsdAnchor ? "usd" : "eth", result,
    };
  }
